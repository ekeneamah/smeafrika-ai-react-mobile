...
export function TrafficReportScreen({ route }: TrafficReportScreenProps) {
  ...
  if (error) {
    return <Label className="text-error">{error}</Label>;
  }
      ...
      <StackLayout className="p-4">
        <GridLayout columns="*, auto" className="mb-4">
          ...
          <DropDown
            ...
            className="w-32"
            onSelectedIndexChanged={(e) => setTimeRange(ranges[e.object.selectedIndex])}
          />
        </GridLayout>
        ...
        <StackLayout className="card">
          ...
          <GridLayout columns="*, *" rows="auto, auto" className="text-center">
            {[
              { title: "12,450", subtitle: "Total Visits" },
              { title: "8,320", subtitle: "Unique Visitors" },
              { title: "2:45", subtitle: "Avg. Time" },
              { title: "3.8", subtitle: "Pages/Visit" }
            ].map((item, idx) => (
              <StackLayout key={idx} col={idx % 2} row={Math.floor(idx / 2)} className="p-2">
                <Label className="text-title">{item.title}</Label>
                <Label className="text-body">{item.subtitle}</Label>
              </StackLayout>
            ))}
          </GridLayout>
        </StackLayout>
        ...
        <StackLayout className="card mt-4">
          ...
          <GridLayout className="h-48 bg-primary100 rounded-md" columns="*">
